...
prepare::
	...
	if [[ "${OS}" != "Darwin" ]]; then \
		sed -i 's,github.com/dirien/pulumi-snowflake,${REPOSITORY},g' provider/go.mod; \
...
build_java:: bin/pulumi-java-gen install_plugins tfgen
	$(WORKING_DIR)/bin/$(JAVA_GEN) generate --schema provider/cmd/$(PROVIDER)/schema.json --out sdk/java --build gradle
	cd sdk/java/ && \
